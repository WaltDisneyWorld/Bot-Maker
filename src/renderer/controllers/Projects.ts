...
export default class Projects {
  ...
  constructor () {
    ...
    if (this.projects.find((p) => p.working === true)) {
      const workingProject = this.projects.find((p) => p.working === true)
      if (existsSync(workingProject.path)) {
        this._working = workingProject
      } else {
        this.remove(workingProject.name)
      }
    }
  }
  ...
  create (name: string, path: string): Promise<void | string> {
    return new Promise((resolve, reject) => {
      ...
      if (!existsSync(path)) return reject('INVALID_PATH')

      const realPath = path + '/' + name

      if (existsSync(realPath)) return reject('PATH_ALREADY_IN_USE')
      if (this.projects.find((p) => p.name === name)) {
        const project = this.projects.find((p) => p.name === name)
        if (existsSync(project.path)) return reject('PROJECT_ALREADY_EXISTS')
      ...
      exec(
        ...
        (err) => {
          if (err || !existsSync(realPath + '/node_modules'))
            return reject('FAILED_TO_INSTALL_PROJECT_DEPENDENCIES')

          if (this.working) {
            this.edit(this.working.name, {
              working: false
            })
          }
